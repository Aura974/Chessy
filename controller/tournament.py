...
class TournamentController:
    def __init__(self):
        name, time_control = v.get_tournament_info()
    ...
    def print_player(self):
        v.print_player(self.tournament.players)
    ...
    def run_other_rounds(self, scoreboard2=None):
        self.scoreboard2 = dict(
            sorted(
                self.scoreboard1.items(), key=lambda item: item[1],
                reverse=True
                )
                                )
        round2 = Round("2")
        ...
        for match in self.tournament.rounds[0].matches:
            ...
            match.score_player1, match.score_player2 = self.handle_score()
            v.print_match_result(match)
            self.scoreboard2[match.player1.name] = match.score_player1
            self.scoreboard2[match.player2.name] = match.score_player2
